...
from utils import is_subjective, is_objective
...
class SubjectivityPredictor(object):
    ...
    def __init__(self, dropout=1.0):
        ...
        with self.g.as_default():
            with tf.variable_scope(NAMESPACE):
                ...
                self.bq = tf.Variable(tf.random_uniform(
                    [self._word_proj_size_for_rnn], -0.1, 0.1))
                self.internal_projection = lambda x: tf.nn.relu(
                    tf.matmul(x, self.Wq) + self.bq)
                self.sentence_int_fw = tf.map_fn(
                    self.internal_projection, self.sentence_vectors_fw)
                self.sentence_int_bw = tf.map_fn(
                    self.internal_projection, self.sentence_vectors_bw)

                self.rnn_cell_fw = rnn.MultiRNNCell(
                    [rnn.GRUCell(self._memory_dim)
                     for _ in range(self._stack_dimension)],
                    state_is_tuple=True)
                ...
                with tf.variable_scope('bw'):
                    _, state_bw = tf.nn.dynamic_rnn(self.rnn_cell_bw, self.sentence_int_bw, time_major=True,
                                                    dtype=tf.float32)
                self.sentence_vector = tf.concat(
                    values=[state_fw[-1], state_bw[-1]], axis=1)

                # Final feedforward layers
                self.Ws1 = tf.Variable(tf.random_uniform([self._memory_dim * 2, self._hidden_dim], -0.1, 0.1),
                                       name='Ws1')
                self.bs1 = tf.Variable(tf.random_uniform(
                    [self._hidden_dim], -0.1, 0.1), name='bs1')
                self.hidden = tf.nn.relu(
                    tf.matmul(self.sentence_vector, self.Ws1) + self.bs1)
                self.hidden_dropout = tf.nn.dropout(self.hidden, dropout)

                self.Wf = tf.Variable(tf.random_uniform(
                    [self._hidden_dim, self._output_size], -0.1, 0.1), name='Wf')
                self.bf = tf.Variable(tf.random_uniform(
                    [self._output_size], -0.1, 0.1), name='bf')
                self.outputs = tf.nn.softmax(
                    tf.matmul(self.hidden_dropout, self.Wf) + self.bf)

                # Loss function and training
                self.y_ = tf.placeholder(tf.float32, shape=(
                    None, self._output_size), name='y_')
                self.one = tf.ones_like(self.outputs)
                self.tiny = self.one * TINY
                self.forbidden = tf.placeholder(tf.float32, shape=(
                    None, None, self._output_size), name='forbidden')
            ...
            capped_gvs = [(tf.clip_by_value(grad, -1., 1.), var)
                          for grad, var in gvs if var.name.find(NAMESPACE) != -1]
    ...
    def __train(self, sentence_vectors, y):
        ...
        loss, _, summary = self.sess.run(
            [self.cross_entropy, self.train_step, self.merged], feed_dict)
        return loss, summary
    ...
    def load_tensorflow(self, filename):
        with self.g.as_default():
            saver = tf.train.Saver(
                [v for v in tf.global_variables() if NAMESPACE in v.name])
